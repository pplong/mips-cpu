...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  | Nop | Li _ | LiL _ | Comment _ | Restore _ as exp -> (Ans(exp), regenv)
  ...
  | Lwcl(x, y', f) -> (Ans(Lwcl(find x Type.Int regenv, find' y' regenv, f)), regenv)
  | Swcl(x, y, z') -> (Ans(Swcl(find x Type.Float regenv, find y Type.Int regenv, find' z' regenv)), regenv)
