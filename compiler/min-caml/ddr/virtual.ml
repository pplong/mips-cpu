(* translation into DDR assembly with infinite number of virtual registers *)
...
let rec g env = function (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  ...
  | Closure.Float(d) ->
	...
	with Not_found ->
	  ...
	  l in
      let x = Id.genid "l" in
      Let((x, Type.Int), LiL(l), Ans(Lwcl(x, C(0), true)))
  ...
  | Closure.Tuple(xs) -> (* 組の生成 (caml2html: virtual_tuple) *)
	...
	expand
	  ...
	  (0, Ans(Mov(y)))
	  (fun x offset store -> seq(Swcl(x, y, C(offset)), store))
  ...
  | Closure.LetTuple(xts, y, e2) ->
	...
	expand
	  ...
	  (fun x offset load ->
	    ...
	    fletd(x, Lwcl(y, C(offset), false), load))
	  (fun x t offset load ->
	    if not (S.mem x s) then load else (* [XX] a little ad hoc optimization *)
  ...
  | Closure.Get(x, y) -> (* 配列の読み出し (caml2html: virtual_get) *)
	  ...
	  Let((offset, Type.Int), Mov(y),
	      Ans(Lwcl(x, V(offset), false)))
      | Type.Array(_) ->
	  Let((offset, Type.Int), Mov(y),
	      Ans(Lw(x, V(offset))))
      | _ -> assert false)
  | Closure.Put(x, y, z) ->
      ...
      | Type.Array(Type.Float) ->
	  Let((offset, Type.Int), Mov(y),
	      Ans(Swcl(z, x, V(offset))))
      | Type.Array(_) ->
	  Let((offset, Type.Int), Mov(y),
	      Ans(Sw(z, x, V(offset))))
      | _ -> assert false)
  | Closure.ExtArray(Id.L(x)) -> Ans(LiL(Id.L("min_caml_" ^ x)))
...
let h { Closure.name = (Id.L(x), t); Closure.args = yts; Closure.formal_fv = zts; Closure.body = e } =
  ...
  let (offset, load) =
    expand
      ...
      (fun z offset load -> fletd(z, Lwcl(reg_cl, C(offset), false), load))
