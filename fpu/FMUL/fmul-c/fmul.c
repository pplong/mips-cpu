...
uint32_t fmul(uint32_t data1, uint32_t data2) {
	fman f1, f2;
	...
	************************************************************/
	...
	uint32_t search = 0x8000000;
//only 28th bit is '1' *maybe not needed in this program

	uint32_t pexp = exp1 + exp2;

//"mts" for "mantissa" (or "fraction of floating expression without 1 on MSB")
	...
	if ((mts & search) == search) {
    //branch if 28th bit of mts is '
		pexp += 1;
		if ((mts & 0x1) == 1) {
			sticky = 1;
			mts = (mts >> 1) | 0x1;
	...
	**************************************/
	...
	// 桁上げ確認
	if ((mts & search) == search) {
    //branch if 28th bit of mts is '1
		pexp += 1;
		mts = mts >> 1;
	}
	/* overflow */
