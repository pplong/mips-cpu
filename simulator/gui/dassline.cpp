...
//Op:11 frtype
    ...
    //FP Add Single:add.s
        if (frl.func == 0x00 && frl.fmt == 0x10) {
            sprintf(cline,"add.s f%u,f%u,f%u",frl.d,frl.s,frl.t);
        }
    //FP Subtract Single:sub.s
        else if (frl.func == 0x01 && frl.fmt == 0x10) {
            sprintf(cline,"sub.s f%u,f%u,f%u",frl.d,frl.s,frl.t);
        }
    //FP Multiply Single:mul.s
        else if (frl.func == 0x02 && frl.fmt == 0x10) {
            sprintf(cline,"mul.s f%u,f%u,f%u",frl.d,frl.s,frl.t);
        }
    //FP Divide Single:div.s
        else if (frl.func == 0x03 && frl.fmt == 0x10) {
            sprintf(cline,"div.s f%u,f%u,f%u",frl.d,frl.s,frl.t);
        }
/*
    //fitype Branch On ~
...
//Op:0x31 itype Load FP Single:lwcl
        ...
        itype il;
        il.line = line;
        sprintf(cline,"lwcl r%u,f%u,%d",il.t,il.s,il.imm);
    }
